...
# 실행 중인 도커 컴포즈 확인
EXIST_A=$(sudo docker-compose -p travelrhythm-a -f docker-compose.a.yml ps | grep Up)
...
echo "travelrhythm-${START_CONTAINER} up"

# 실행해야하는 컨테이너 docker-compose로 실행. -p는 docker-compose 프로젝트에 이름을 부여
# -f는 docker-compose파일 경로를 지정
sudo docker-compose -p travelrhythm-${START_CONTAINER} -f docker-compose.${START_CONTAINER}.yml up -d --build
...
do
        ...
        UP=$(curl -s http://127.0.0.1:${START_PORT}/api/v1/hello | grep 'hello')
...
echo "travelrhythm-${TERMINATE_CONTAINER} down"
sudo docker-compose -p travelrhythm-${TERMINATE_CONTAINER} -f docker-compose.${TERMINATE_CONTAINER}.yml down
